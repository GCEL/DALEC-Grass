...
!
! A simple process based model of carbon (C) dynamics in managed grassland ecosystems (temperate ryegrass/clover dominated). 
...
! ----------------------------------------------------------------------------------------------------------------
! POOLS:   1.labile/stem 2.foliar 3.root 4.litter 5.SOM    ! PARAMETERS:                                
...
!          14.litter2som                                   ! 15. LCA (g.C.leaf_m-2) -OR- slope value for LCA = -slope*LAI+intercept  
...
!                                                          ! 21. GSI min VPD threshold (Pa) 
!                                                          ! 22. GSI max VPD threshold (Pa)
...
!          7.cutting/grazing :                             ! 30. C SOM (initialization)
!          |-> spatial mode = LAI removed (m2.m-2)         ! 31. DM demand of animal weight (fraction)
!          |-> field mode = LSU.ha-1                       ! 32. Post-grazing labile loss (fraction)
!          8.21-day avg min T (K)                          ! 33. Post-cut labile loss (fraction)
!          9.21-day avg photoperiod (sec)                  ! 34. Minimum grazed biomass to allow grazing
...
!
  subroutine CARBON_MODEL(start,finish,deltat,lat,met,pars &
    ...
    do n = start, finish  
      ...
      FLUXES(n,12) = POOLS(n,4)*(1.-(1.-FLUXES(n,2)*pars(7))**deltat(n))/deltat(n)
      ...
      FLUXES(n,14) = POOLS(n,4)*(1.-(1.-pars(1)*FLUXES(n,2))**deltat(n))/deltat(n)
      ...
      if (version_code .EQ. 2) then 
        ...
        ! CUTTING (if : AGB > cutting limit & met(7,n) = 100 i.e. cutting code)
