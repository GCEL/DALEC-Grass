  ...
  subroutine CARBON_MODEL(start,finish,deltat,lat,met,pars &
    ...
    do n = start, finish
      ...
      if (version_code .EQ. 1) then 
        ...
        ! CUTTING (if AGB > limit & April-October & LAI_loss > cut DM/LMA & no cut last month)
        if ( ( (POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(28)*0.475*0.1) )  & 
             .AND. ( met(6,n) .GE. 90 ) .AND. ( met(6,n) .LE. 304 ) & 
             .AND. ( ( met(8,n) .GE. ((pars(28)-pars(27))*0.475*0.1)/pars(15) )  & 
             .OR. ( (met(8,n) + met(8,n+1)) .GE. ((pars(28)-pars(27))*0.475*0.1)/pars(15) )    &
             .OR. ( (met(8,n) + met(8,n+1) + met(8,n+2)) .GE. ((pars(28)-pars(27))*0.475*0.1)/pars(15) ) ) & 
            ...
            ! if removed > 1500 kg.DM.ha-1 then proceed with cut 
        ...
        ! GRAZING (if remfrac > 0 & AGB > limit & no cutting this and 1 and 2 weeks ago)
        if ( (met(8,n) > 0.) .AND. ( (POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(27)*0.475*0.1) ) & 
            ...
            foliar_loss  = max(0.,met(8,n) * pars(15) - labile_loss)
            roots_loss   = 0 ! POOLS(n+1,3) * roots_frac_death
