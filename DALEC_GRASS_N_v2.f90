! ----------------------------------------------------------------------------------------------------------------
! THE DALEC-GRASS MODEL 
!
! A simple process based model of carbon (C) and nitrogen (N) dynamics 
! in managed grassland ecosystems (temperate ryegrass/clover dominated). 
! Developed from a generic-vegetation DALEC (Data Assimilation and Modelling of the Carbon Cycle) model
! N cycling is based on C:N ratio of pools (plant tissues, litter, SOM)
! 
! The model can be run in 2 modes : 
! (1) Field mode   : Info on vegetation management (grazing,harvesting) is provided by the user 
! (2) Spatial mode : Info on vegetation management (grazing,harvesting) is inferred from earth observation (EO) data 
! 
! The model is integrated into a Model-Data Fusion (MDF) algorithm (python) that :
! 1. Assimilates observational data e.g. EO based time-series on Leaf Area Index, tower-measured net ecosystem exchange
! 2. Runs the model forward to provide estimates of C pools/fluxes and grass grazing/yields
!
! Input and (EO-based) observational data can be created using accompaning code (DALEC_Grass_insNobs.py)
! 
! Vasilis Myrgiotis - 2019 - vmyrgiotis.github.io
! ----------------------------------------------------------------------------------------------------------------
! POOLS:                                                   !
! Carbon:   1.labile/stem 2.foliar 3.root 4.litter 5.SOM   ! 
! Nitrogen: 6.labile/stem 7.foliar 8.root 9.litter 10.SOM  !                                          
!                                                          ! PARAMETERS: 
! ---------------------------------------------------------! 1.  Decomposition rate
! FLUXES:  1.Gross Primary Production (GPP)                ! 2.  Fraction of GPP respired 
! (daily)  2.temperature rate                              ! 3.  GSI sensitivity for leaf growth
!          3.autotrophic respiration                       ! 4.  NPP belowground allocation parameter
!          4.leaf production                               ! 5.  GSI max leaf turnover 
!          5.labile/stem production                        ! 6.  TOR (turnover rate) of roots
!          6.root production                               ! 7.  TOR of litter
!          7.aboveground production                        ! 8.  TOR of SOM
!          8.labile consumption -> leaves                  ! 9.  Temperature factor Q10 (1.2-1.6)
!          9.leaffall factor                               ! 10. Photosynthetic N use efficiency
!          10.leaf litter production                       ! 11. GSI max labile turnover
!          11.rootlitter production                        ! 12. GSI min temperature threshold (K)
!          12.respiration het litter                       ! 13. GSI max temperature threshold (K)
!          13.respiration het som                          ! 14. GSI min photoperiod threshold (sec)
!          14.litter2som                                   ! 15. LCA (g.C.leaf_m-2) 
!          15.labrelease factor(leaf growth)               ! 16. C labile/stem (initialization)
!          16.growing season index (GSI)                   ! 17. C foliar (initialization)
!          17.animal manure C soil input (per time step)   ! 18. C roots  (initialization)
!          18.animal resp CO2 (per time step)              ! 19. C litter (initialization)
!          19.animal CH4 (per time step)                   ! 20. GSI max photoperiod threshold (sec)
!          20.litter N loss to atm                         ! 21. GSI min VPD threshold (Pa)    
!          21.litter N loss to SOM                         ! 22. GSI max VPD threshold (Pa)
!          22.fertiliser-N use                             ! 23. critical GPP for LAI increase (gC.m-2.day-1)  
!          23.labile to leaf N                             ! 24. GSI senstivity for leaf senescence 
!          24.leaf litter N                                ! 25. GSI - have I just left a growing state (>1)  
!          25.root litter N                                ! 26. GSI - initial GSI value    
!          26.SOM N loss to atm                            ! 27. DM min lim for grazing (kg.DM.ha-1)
!                                                          ! 28. DM min lim for cutting (kg.DM.ha-1)
!                                                          ! 29. leaf-vs-stem allocation factor 
!                                                          ! 30. C SOM (initialization)
! ---------------------------------------------------------! 31. DM demand of animal weight (fraction)
! MET:     1.run day                                       ! 32. Post-grazing labile loss (fraction)
!          2.min T (C)                                     ! 33. Post-cut labile loss (fraction)
!          3.max T (C)                                     ! 34. Minimum grazed biomass to allow grazing
!          4.Radiation (MJ.m-2)                            ! 35. Harvest leaf % reduction 
!          5.CO2 (ppm)                                     ! 36. grazed N return to litter (fraction 0.3-0.6) 
!          6.DOY                                           ! 
!          7.cutting/grazing :                             ! 
!          |-> spatial mode = LAI removed (m2.m-2)         ! 
!          |-> field mode = LSU.ha-1                       ! 
!          8.21-day avg min T (K)                          ! 
!          9.21-day avg photoperiod (sec)                  ! 
!          10.21-day avg VPD (Pa)                          ! 
! ----------------------------------------------------------------------------------------------------------------
! NOTES : 1 LSU per ha = 1 cow that weighs 650kg and grazes on 1 ha of grassland 
!         carbon = 0.475 * dry matter 
!         1 g.C.m-2 = 0.021 t.DM.ha-1 = 21 kg.DM.ha-1
...
!
  subroutine CARBON_MODEL(start,finish,deltat,lat,met,pars &
    ...
    if (start == 1) then
        ...
        ! carbon pools from parameters 
        POOLS(1,1) = pars(16)
        POOLS(1,2) = pars(17)
        POOLS(1,3) = pars(18)
        POOLS(1,4) = pars(19)
        POOLS(1,5) = pars(30)
        ! nitrogen pools from C pools / pool_CN_ratio
        POOLS(1,7)  = POOLS(1,2) / 15  ! leaf N=C/CN
        POOLS(1,8)  = POOLS(1,1) / 30  ! stem N=C/CN
        POOLS(1,9)  = POOLS(1,3) / 40  ! roots N=C/CN
        POOLS(1,10) = POOLS(1,4) / 10  ! litter N=C/CN
        POOLS(1,11) = POOLS(1,5) / 50  ! SOM N=C/CN (assuming recalcitrance; very high C:N)
    ...
    do n = start, finish  
      ...
      ! ! dynamic LMA : LMA(n) = slope * LAI(n-1) + intercept  
      ! if (n == 1) then
      !   LMA = 70 
      ! else 
      !   LMA = (-pars(15))*LAI(n-1) + 70 
      ! endif
      ...
      ! Tfac = (met(8,n)-(pars(12)-273.15)) / (pars(13)-pars(12)) ! no need to K->C 
      Tfac = ( met(8,n)-pars(12)) / (pars(13)-pars(12) )
      Tfac = min(1d0,max(0d0,Tfac))
      ! photoperiod limitation
      Photofac = ( met(9,n)-pars(14)) / (pars(20)-pars(14) )
      Photofac = min(1d0,max(0d0,Photofac))
      ! VPD limitation
      VPDfac = 1.0 - ( (met(10,n)-pars(21)) / (pars(22)-pars(21)) )
      VPDfac = min(1d0,max(0d0,VPDfac))

      ! calculate and store the GSI index
      FLUXES(n,16) = Tfac * Photofac * VPDfac
      ...
      if (n == 1) then
          ...
          gsi_history(gsi_lag) = FLUXES(n,16)
      else
      ...
      ! C:N ratio of pools 
      leafCN   = POOLS(n,2) / POOLS(n,6)
      stemCN   = POOLS(n,1) / POOLS(n,7)
      rootCN   = POOLS(n,3) / POOLS(n,8)
      litterCN = POOLS(n,4) / POOLS(n,9)
      somCN    = POOLS(n,5) / POOLS(n,10)

      ! FLUXES WITH TIME DEPENDENCIES

      ! labile release = P_labile * (1-(1-leafgrowth)**deltat)/deltat
      FLUXES(n,8)  = POOLS(n,1)*(1.-(1.-FLUXES(n,15))**deltat(n))/deltat(n)
      FLUXES(n,X) = FLUXES(n,X) / leafCN      
      ! leaf litter production = P_foliar * (1-(1-leaffall)**deltat)/deltat  
      FLUXES(n,10) = POOLS(n,2)*(1.-(1.-FLUXES(n,9))**deltat(n))/deltat(n)
      FLUXES(n,X) = FLUXES(n,X) / stemCN
      !  root litter production = P_root * (1-(1-rootTOR)**deltat)/deltat  
      FLUXES(n,11) = POOLS(n,3)*(1.-(1.-pars(6))**deltat(n))/deltat(n)
      FLUXES(n,X) = FLUXES(n,X) / rootCN

      ! FLUXES WITH TEMP AND TIME DEPENDENCIES

      ! resp het litter = P_litter * (1-(1-GPP_respired*litterTOR)**deltat)/deltat  
      FLUXES(n,11) = POOLS(n,4)*(1.-(1.-FLUXES(n,2)*pars(7))**deltat(n))/deltat(n)
      FLUXES(n,X) = FLUXES(n,12) / litterCN
      ! resp het som = P_som * (1-(1-GPP_respired*somTOR)**deltat)/deltat
      FLUXES(n,12) = POOLS(n,5)*(1.-(1.-FLUXES(n,2)*pars(8))**deltat(n))/deltat(n)
      FLUXES(n,X) = FLUXES(n,13) / somCN
      ! litter to som = P_litter * (1-(1-dec_rate*temprate)**deltat)/deltat
      FLUXES(n,13) = POOLS(n,4)*(1.-(1.-pars(1)*FLUXES(n,2))**deltat(n))/deltat(n)
      FLUXES(n,X) = FLUXES(n,14) / litterCN
     

      ! NEE = resp_auto + resp_het_litter + resp_het_som - GPP [i.e. '-' when CO2 sink '+' when CO2 source ]
      NEE(n) = (FLUXES(n,3) + FLUXES(n,12) + FLUXES(n,13)) - FLUXES(n,1)
      ! GPP 
      GPP(n) = FLUXES(n,1)
      ...
      POOLS(n+1,6) = POOLS(n,6) + (FLUXES(n,X) - FLUXES(n,X))*deltat(n) ! N leaf litter and from-stem transfer
      ...
      POOLS(n+1,7) = POOLS(n,7) - (FLUXES(n,X))*deltat(n) ! N transfer to leaf
      ! root pool = root_pool[†-1] + (root_prod - root_litter_prod)*deltat
      POOLS(n+1,3) = POOLS(n,3) + (FLUXES(n,6)-FLUXES(n,11))*deltat(n)
      POOLS(n+1,8) = POOLS(n,8) - (FLUXES(n,X))*deltat(n) ! root litter production
      ! litter pool = litter_pool[†-1] + (leaf_litter_prod + root_litter_prod - resp_het_litter - litter2som)*deltat
      POOLS(n+1,4) = POOLS(n,4) + (FLUXES(n,10)+FLUXES(n,11)-FLUXES(n,12)-FLUXES(n,14))*deltat(n)
      POOLS(n+1,9) = POOLS(n,9) + (FLUXES(n,X)+FLUXES(n,X)-FLUXES(n,X)-FLUXES(n,X))*deltat(n) ! fluxes to atm and SOM     
      ! som pool = som_pool[†-1] + (litter2som - resp_het_som + wood_litter_prod)
      POOLS(n+1,5) = POOLS(n,5) + (FLUXES(n,14)-FLUXES(n,13))*deltat(n)
      POOLS(n+1,10) = POOLS(n,10) + (FLUXES(n,X)-FLUXES(n,X))*deltat(n)
      ...
      ! ------------- SPATIAL MODE : CUTTING --------------------------------------------------- ! 
        
        ! conditions to be met before considering LAI reduction input as caused by cutting
        if ( ((POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(28)*0.0475))   &  ! AGB >= pre-cutting limit 
             ! .AND. ( met(6,n) .GE. 120 ) .AND. ( met(6,n) .LE. 273 ) & ! May >= date >= Sep
             ! .AND. ( LAI(n) .GE. 4 ) &                                 ! LAI >= 4 m2.m-2
             ! .AND. ( met(7,n) .GE. 2 ) &                               ! removed LAI >= 2 m2.m-2
             .AND. ( (met(7,n) * LMA * 0.021) .GE. 1 ) &                 ! removed leaf biomass >= 1 tDMha-1
             ! .AND. (REMOVED_C(2,n-1) .EQ. 0.0) .AND. (REMOVED_C(2,n-2) .EQ. 0.0 ) & ! no cut in the previous 4 weeks 
             ! .AND. (REMOVED_C(2,n-3) .EQ. 0.0) .AND. (REMOVED_C(2,n-4) .EQ. 0.0 )   ! no cut in the previous 4 weeks 
             ) then 

            ! direct C lossess
            labile_loss  = POOLS(n+1,1) * pars(33) ! % of stem biomass lost after cutting 
            foliar_loss  = POOLS(n+1,2) * 0.95     ! % of leaf biomass lost after cutting 
            ...
            ! if havest will yield > XXXX kg.DM.ha-1 proceed with cut -- else do not simulate
            if ( ((foliar_loss-foliar_residue)+(labile_loss-labile_residue)) .GE. (2000*0.0475) ) then
              ...
              POOLS(n+1,4) = max(0., POOLS(n+1,4) + (labile_residue+foliar_residue+roots_loss))
              POOLS(n+1,5) = max(0., POOLS(n+1,5))
              POOLS(n+1,6) = POOLS(n+1,6) - foliar_loss/leafCN ! leaf N after grazing
              POOLS(n+1,7) = POOLS(n+1,7) - labile_loss/stemCN ! stem N after grazing
              POOLS(n+1,9) = POOLS(n+1,9) + foliar_residue/leafCN + labile_residue/stemCN ! litter N after plant residues addition 

            endif 

        endif ! end spatial mode cutting process   
        
      ! ------------- SPATIAL MODE : GRAZING --------------------------------------------------- ! 

      ! conditions to be met before considering LAI reduction input as caused by grazing        
      if ( ( (POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(27)*0.0475) )  & ! AGB > pregraze limit
         .AND. (met(7,n) .GE. 0) &                                   ! LAI removal > 0 
         .AND. (met(2,n) .GE. 0) &                                   ! min_T > 0C 
         ! .AND. (REMOVED_C(2,n-1) .EQ. 0.0) .AND. (REMOVED_C(2,n-2) .EQ. 0.0 ) &  ! no cut in the previous 4 weeks 
         ! .AND. (REMOVED_C(2,n-3) .EQ. 0.0) .AND. (REMOVED_C(2,n-4) .EQ. 0.0 )    ! no cut in the previous 4 weeks 
         ) then
          ...
          foliar_loss  = max(0.,(met(7,n) * LMA) - labile_loss) 
          ...
          if ( (((POOLS(n+1,2)+POOLS(n+1,1))-foliar_loss-labile_loss) .GE. (pars(27)*0.0475)) & 
              ...
              FLUXES(n,17) = REMOVED_C(1,n) * 0.32
              ! animal respiration CO2-C
              FLUXES(n,18) = REMOVED_C(1,n) * 0.54
              ! animal CH4-C 
              FLUXES(n,19) = REMOVED_C(1,n) * 0.04 

              ! update pools 
              POOLS(n+1,1) = max(0.,POOLS(n+1,1)-labile_loss)
              POOLS(n+1,2) = max(0.,POOLS(n+1,2)-foliar_loss)
              POOLS(n+1,3) = max(0.,POOLS(n+1,3)-roots_loss)
              POOLS(n+1,4) = max(0., POOLS(n+1,4) + (labile_residue+foliar_residue+roots_loss) + FLUXES(n,17))
              POOLS(n+1,5) = max(0., POOLS(n+1,5))
              POOLS(n+1,6) = POOLS(n+1,6) - foliar_loss/leafCN ! leaf N after grazing
              POOLS(n+1,7) = POOLS(n+1,7) - labile_loss/stemCN ! stem N after grazing
              
              ! litter N after plant residues and livestock manure/urine additions   
              POOLS(n+1,9) = POOLS(n+1,9) + foliar_residue/leafCN + labile_residue/stemCN & 
                            + pars(36)*((labile_loss-labile_residue)/stemCN + (foliar_loss-foliar_residue)/leafCN)      
          endif 
          ...
          !       POOLS(n+1,2) = max(0.,POOLS(n+1,2)-foliar_loss)
          !       POOLS(n+1,3) = max(0.,POOLS(n+1,3)-roots_loss)
          !       POOLS(n+1,4) = max(0., POOLS(n+1,4) + (labile_residue+foliar_residue+roots_loss) + FLUXES(n,18) )
          !       POOLS(n+1,5) = max(0., POOLS(n+1,5))
          !     endif 

          ! endif 

        endif ! end spatial mode grazing process
      ...
      ! ------------- FIELD MODE : CUTTING --------------------------------------------------- ! 
        ...
        !  if : AGB > cutting limit & met(7,n) = 100 i.e. cutting code)
        if ( ((POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(28)*0.0475)) .AND. (met(7,n) .EQ. 100) ) then
        ...
        endif ! end field mode cutting process

      ! ------------- FIELD MODE : GRAZING --------------------------------------------------- ! 
        
        ! (if LSU.ha-1 > 0 & AGB > limit )
        if ( (met(7,n) > 0.0) .AND. (met(7,n) .NE. 100 ) .AND. ((POOLS(n+1,2)+POOLS(n+1,1)) .GE. (pars(27)*0.0475)) ) then
            
            ! direct C losses
            labile_loss  = POOLS(n+1,1) * pars(32)
            ! Remove demand (g.C.m-2) from foliage : LSU.per.ha * 650 kg_weight * 2.5% * convert_kg.DM.ha-1_to_g.C.m-2 
            foliar_loss  = max(0.,met(7,n) * 650 * pars(31) * 0.047619 - labile_loss)
            roots_loss   = 0 ! POOLS(n+1,3) * roots_frac_death
        ...
        endif ! end field mode grazing process
      
      endif ! end version_code check 


      ! ------------------------------------------------------------------------------------------------------------- ! 
      !                            PLANT-N EXCESS/DEMAND AND SOIL-N UPTAKE CALCULATIONS                                                      !
      ! ------------------------------------------------------------------------------------------------------------- ! 
      ...
      ! N demand per tissue 
      stemN_demand  = (POOLS(n+1,1)/30) - POOLS(n+1,6) 
      leafN_demand  = (POOLS(n+1,2)/15) - POOLS(n+1,7) 
      rootsN_demand = (POOLS(n+1,3)/40) - POOLS(n+1,8) 
      ...
      if ( (sumN_demand > 0) ) then 
            POOLS(n+1,9) = POOLS(n+1,9) - sumN_demand + sumN_excess
            POOLS(n+1,6)  = POOLS(n+1,6) + leafN_demand - leafN_excess
            POOLS(n+1,7)  = POOLS(n+1,7) + stemN_demand - stemN_excess
            POOLS(n+1,8)  = POOLS(n+1,8) + rootsN_demand - rootsN_excess
      endif 
      
      ! if litter C:N > 20 -> litter C:N=7 with fertiliser application
      if (POOLS(n+1,4)/(POOLS(n+1,9)) .GE. 15)  then 
         FLUXES(n+1,X) = (POOLS(n+1,4)/7) - POOLS(n+1,8) ! fertiliser N g.N.m-2
         POOLS(n+1,9) = POOLS(n+1,9) + FLUXES(n+1,X)
      endif  

      if (POOLS(n+1,4)/(POOLS(n+1,9)) < 15)  then 
         FLUXES(n+1,X) = 0 ! fertiliser N = 0 g.N.m-2
      endif  
